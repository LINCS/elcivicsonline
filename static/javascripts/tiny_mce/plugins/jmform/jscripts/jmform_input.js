...
var action;
var tempName = '';
function setResetButton(on) {
    var formObj = document.forms[0];
    if (on == 'reset') {
        formObj.elements['jmformName'].value = 'reset';
        formObj.elements['jmformName'].disabled = 'true';
    }
    else {
        formObj.elements['jmformName'].value = tempName;
        formObj.elements['jmformName'].disabled = '';
    }
}
function insertInput() {
	...
	onkeyup = formObj.elements['onkeyup'].value;
	onkeydown = formObj.elements['onkeydown'].value;
	...
	if (action == "update") {
		...
		var elm = tinyMCE.getParentElement(focusElm, "input");
		...
		tinyMCE.setAttrib(elm, 'onkeyup', onkeyup, true);
		tinyMCE.setAttrib(elm, 'onkeydown', onkeydown, true);
	...
	html += makeAttrib('onkeyup', onkeyup);
	html += makeAttrib('onkeydown', onkeydown);
...
function init() {
	...
	var inst = tinyMCE.selectedInstance;
	var inpElm = tinyMCE.getParentElement(inst.getFocusElement(), "input");
	var formObj = document.forms[0];
    var jmformName = '', jmformValue = ''; width = '', height = '', jmformType = '', size = '', maxlength = '', disabled = '', required = '', checked = '';
    var src = '', onfocus = '', onblur = '', onchange = '', onselect = '', onclick = '', onkeyup = '', onkeydown = '';
    var st = '', bordercolor = '', bgcolor = '', backgroundimage = '', id = '', className = '';
    
    if (inpElm != undefined) {
    	st = tinyMCE.parseStyle(tinyMCE.getAttrib(inpElm, "style"));
        className = tinyMCE.getVisualAidClass(tinyMCE.getAttrib(inpElm, 'class'), false);
    		
    	// Get input element data
    	jmformName = tinyMCE.getAttrib(inpElm, 'name');
    	jmformValue = tinyMCE.getAttrib(inpElm, 'value');
    	width = trimSize(getStyle(inpElm, 'width', 'width'));
    	height = trimSize(getStyle(inpElm, 'height', 'height'));
    	jmformType = tinyMCE.getAttrib(inpElm, 'type');
    	size = tinyMCE.getAttrib(inpElm, 'size');
    	maxlength = tinyMCE.getAttrib(inpElm, 'maxlength');
    	required = tinyMCE.getAttrib(inpElm, 'required');
    	checked = tinyMCE.getAttrib(inpElm, 'checked');
    	src = tinyMCE.getAttrib(inpElm, 'src');
    	disabled = tinyMCE.getAttrib(inpElm, 'disabled');
    	onfocus = tinyMCE.getAttrib(inpElm, 'onfocus');
    	onblur = tinyMCE.getAttrib(inpElm, 'onblur');
    	onchange = tinyMCE.getAttrib(inpElm, 'onchange');
    	onselect = tinyMCE.getAttrib(inpElm, 'onselect');
    	onclick = tinyMCE.getAttrib(inpElm, 'onclick');
    	onkeyup = tinyMCE.getAttrib(inpElm, 'onkeyup');
    	onkeydown = tinyMCE.getAttrib(inpElm, 'onkeydown');
    	...
    	id = tinyMCE.getAttrib(inpElm, 'id');
    }
...
function updateElem(inp, skip_id) {
	...
	inp.setAttribute('style', tinyMCE.serializeStyle(tinyMCE.parseStyle(formObj.style.value)));
